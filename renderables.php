...
class attendance_tabs implements renderable {
    ...
    const TAB_ABSENTEE      = 9;
      /** Add Modification tab */
    const TAB_MODIFICATIONADD      = 10;
    /** more reports tab */
    const TAB_MOREREPORTS      = 11;
     /** more reports tab */
    const TAB_MISSEDSESSIONS      = 12;
    ...
    public function get_tabs() {
        ...
        if (has_capability('mod/attendance:modifications', $context)) {
            //tabs for modifications
            $toprow[] = new tabobject(self::TAB_MODIFICATIONADD, $this->att->url_modificationadd()->out(),
            'Modifications');
        }

        if (has_capability('mod/attendance:morereports', $context)) {
             //tabs for more reports
             $toprow[] = new tabobject(self::TAB_MOREREPORTS, $this->att->url_morereports()->out(),
                             'SALT Reports');
        }

        if (has_capability('mod/attendance:changeattendancesafterdeadline', $context)) {
            //tabs for view missed attendance sessions
             $toprow[] = new tabobject(self::TAB_MISSEDSESSIONS, $this->att->url_missedsessions()->out(),
                             'Missed Sessions');
         }
...
class attendance_filter_controls implements renderable {
    ...
    public function __construct(mod_attendance_structure $att, $report = false) {
        ...
        // $currentgroupid=groups_get_group_by_name($att->cm->course, 'current');

        // $this->currentid=$currentgroupid;
        ...
        if ($this->pageparams->mode == mod_attendance_view_page_params::MODE_THIS_COURSE) {
            ...
            $this->groups = groups_get_all_groups($att->course->id);
                ...
                foreach ($this->sessionslog as $sessid => $sess) {
                    if(isset($this->coursesatts[$sess->attendanceid]->cmid)){
