<?php
  ...
  */
 define('TABLE_VAR_SORT1',   1);
 define('TABLE_VAR_HIDE1',   2);
 define('TABLE_VAR_SHOW1',   3);
 define('TABLE_VAR_IFIRST1', 4);
 define('TABLE_VAR_ILAST1',  5);
 define('TABLE_VAR_PAGE1',   6);
 define('TABLE_VAR_RESET1',  7);
 define('TABLE_VAR_DIR1',    8);
 /**#@-*/
  ...
  */
 define('TABLE_P_TOP_LOCAL',    1);
 define('TABLE_P_BOTTOM_LOCAL', 2);
 /**#@-*/
 ...
 class flexible_table_local {
      ...
      */
     var $showdownloadbuttonsat= array(TABLE_P_TOP_LOCAL);
     ...
     function __construct($uniqueid) {
         ...
         $this->request  = array(
             TABLE_VAR_SORT1   => 'tsort',
             TABLE_VAR_HIDE1   => 'thide',
             TABLE_VAR_SHOW1   => 'tshow',
             TABLE_VAR_IFIRST1 => 'tifirst',
             TABLE_VAR_ILAST1  => 'tilast',
             TABLE_VAR_PAGE1   => 'page',
             TABLE_VAR_RESET1  => 'treset',
             TABLE_VAR_DIR1   => 'tdir',
         );
     }
     ...
     function export_class_instance($exportclass = null) {
         if (!is_null($exportclass)) {
             ...
             $this->exportclass = new table_dataformat_export_format_local($this, $this->download);
     ...
     protected function is_resetting_preferences(): bool {
         if ($this->resetting === null) {
             $this->resetting = optional_param($this->request[TABLE_VAR_RESET1], false, PARAM_BOOL);
         }
     ...
     function setup() {
         ...
         if ($this->currpage == null) {
             $this->currpage = optional_param($this->request[TABLE_VAR_PAGE1], 0, PARAM_INT);
         }
     ...
     function print_initials_bar() {
         ...
         if ((!empty($ifirst) || !empty($ilast) ||$this->use_initials)
                 && isset($this->columns['fullname'])) {
             $prefixfirst = $this->request[TABLE_VAR_IFIRST1];
             $prefixlast = $this->request[TABLE_VAR_ILAST1];
     ...
     function finish_html() {
         ...
         if (!$this->started_output) {
             ...
             // Paging bar
             if(in_array(TABLE_P_BOTTOM_LOCAL, $this->showdownloadbuttonsat)) {
                 echo $this->download_buttons();
             }

             if($this->use_pages) {
                 $pagingbar = new paging_bar($this->totalrows, $this->currpage, $this->pagesize, $this->baseurl);
                 $pagingbar->pagevar = $this->request[TABLE_VAR_PAGE1];
     ...
     protected function show_hide_link($column, $index) {
         ...
         if (!empty($this->prefs['collapse'][$column])) {
             $linkattributes = array('title' => get_string('show') . ' ' . strip_tags($this->headers[$index]),
                                     ...
                                     'data-action' => 'show',
                                     'data-column' => $column);
             return html_writer::link($this->baseurl->out(false, array($this->request[TABLE_VAR_SHOW1] => $column)),
             ...
             $linkattributes = array('title' => get_string('hide') . ' ' . strip_tags($this->headers[$index]),
                                     ...
                                     'data-action' => 'hide',
                                     'data-column' => $column);
             return html_writer::link($this->baseurl->out(false, array($this->request[TABLE_VAR_HIDE1] => $column)),
     ...
     protected function set_sorting_preferences(): void {
         ...
         if ($sortdata === null) {
             ...
             $sortorder = optional_param($this->request[TABLE_VAR_DIR1], $this->sort_default_order, PARAM_INT);
             $sortby = optional_param($this->request[TABLE_VAR_SORT1], '', PARAM_ALPHANUMEXT);
     ...
     protected function set_initials_preferences(): void {
         ...
         if ($ifirst === null) {
             $ifirst = optional_param($this->request[TABLE_VAR_IFIRST1], null, PARAM_RAW);
         }

         if ($ilast === null) {
             $ilast = optional_param($this->request[TABLE_VAR_ILAST1], null, PARAM_RAW);
         }
     ...
     protected function set_hide_show_preferences(): void {
         ...
         if ($this->hiddencolumns !== null) {
             ...
             if ($column = optional_param($this->request[TABLE_VAR_HIDE1], '', PARAM_ALPHANUMEXT)) {
                 if (isset($this->columns[$column])) {
         ...
         if ($column = optional_param($this->request[TABLE_VAR_SHOW1], '', PARAM_ALPHANUMEXT)) {
     ...
     protected function initialise_table_preferences(): void {
         ...
         if (($showcol = optional_param($this->request[TABLE_VAR_SHOW1], '', PARAM_ALPHANUMEXT)) &&
             isset($this->columns[$showcol])) {
             $this->prefs['collapse'][$showcol] = false;
         } else if (($hidecol = optional_param($this->request[TABLE_VAR_HIDE1], '', PARAM_ALPHANUMEXT)) &&
     ...
     protected function sort_link($text, $column, $isprimary, $order) {
         ...
         $params = [
             $this->request[TABLE_VAR_SORT1] => $column,
             $this->request[TABLE_VAR_DIR1] => $sortorder,
         ];
     ...
     function start_html() {
         ...
         if (in_array(TABLE_P_TOP_LOCAL, $this->showdownloadbuttonsat)) {
             echo $this->download_buttons();
         }
     ...
     protected function render_reset_button() {
         ...
         $url = $this->baseurl->out(false, array($this->request[TABLE_VAR_RESET1] => 1));
  ...
  * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
  */
 class table_sql_local extends flexible_table_local {
     ...
     function query_db($pagesize, $useinitialsbar=true) {
         ...
         if (!$this->is_downloading()) {
             if ($this->countsql === NULL) {
                 $this->countsql = 'SELECT COUNT(distinct u.id) FROM '.$this->sql->from.' WHERE '.$this->sql->where;
  ...
  * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
  */
 class table_default_export_format_parent_local {
     /**
  ...
  * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
  */
 class table_dataformat_export_format_local extends table_default_export_format_parent_local {
